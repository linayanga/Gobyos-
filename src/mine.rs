                ...
                while let Some(msg) = message_receiver.recv().await {
                    match msg {
                        ServerMessage::StartMining(challenge, nonce_range, cutoff) => {
                            *last_activity_clone.lock().await = Instant::now();
                            println!("Moian , Cikcji: {} - {}", nonce_range.start, nonce_range.end);
                            ...
                            println!("Jarort: {} , Parksa:{} , Werkomosa:{:?} , Halsiajh:{:.3} k", best_difficulty,total,hash_time , ((total as f32)/hash_time.as_secs_f32())/1000.0 );
